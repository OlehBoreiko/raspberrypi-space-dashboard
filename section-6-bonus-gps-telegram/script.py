...
def iss_flyover():
    try:
        ...
        if not os.path.exists(tle_path):
            import requests
            tle_data = requests.get("https://celestrak.org/NORAD/elements/stations.txt").text
            with open(tle_path, "w") as f:
                f.write(tle_data)

        sats = load.tle_file(tle_path)
        iss = {sat.name: sat for sat in sats}.get("ISS (ZARYA)")
        if not iss:
            return jsonify({"error": "ISS not found in TLE data"}), 500
        ...
        for i in range(0, len(events), 3):
            try:
                ...
                peak = times[i+1].utc_datetime()
                set_ = times[i+2].utc_datetime()
                kyiv = ZoneInfo("Europe/Kyiv")
                grouped.append({
                    "rise": rise.astimezone(kyiv).strftime("%Y-%m-%d %H:%M:%S"),
                    "peak": peak.astimezone(kyiv).strftime("%Y-%m-%d %H:%M:%S"),
                    "set": set_.astimezone(kyiv).strftime("%Y-%m-%d %H:%M:%S"),
                    "duration_seconds": int((set_ - rise).total_seconds())
                })
            except IndexError:
                continue

        return jsonify(grouped[0] if grouped else {"error": "No upcoming pass found"})
    except Exception as e:
...
@app.route("/ai/iss-tracker", methods=["POST"])
def ai_iss_flyover():
    try:
        lat, lon = get_gps_coordinates()
        ts = load.timescale()
        ...
        observer = wgs84.latlon(lat, lon)
        t0 = ts.now()
        t1 = ts.utc(datetime.now().replace(tzinfo=utc) + timedelta(days=2))
        times, events = iss.find_events(observer, t0, t1, altitude_degrees=10.0)

        if len(times) < 3:
            return jsonify({"error": "No upcoming ISS pass found"}), 500

        rise = times[0].utc_datetime().astimezone(ZoneInfo("Europe/Kyiv")).strftime("%Y-%m-%d %H:%M:%S")
        peak = times[1].utc_datetime().astimezone(ZoneInfo("Europe/Kyiv")).strftime("%Y-%m-%d %H:%M:%S")
        set_ = times[2].utc_datetime().astimezone(ZoneInfo("Europe/Kyiv")).strftime("%Y-%m-%d %H:%M:%S")

        prompt = f"The ISS will rise at {rise}, reach peak at {peak}, and set at {set_}. Explain what this means and how users can observe it."

        res = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a space educator. Provide a concise, friendly explanation."},
                {"role": "user", "content": prompt}
            ]
        )
        return jsonify({"text": res.choices[0].message.content})
...
def get_gps_coordinates():
    """
    Returns latitude and longitude from GPS module (/dev/ttyACM0).
    
    👉 If you don't have a GPS module, you can hardcode your coordinates in `except` block below
    ...
    try:
        ...
        return 50.4501, 30.5234  # Kyiv, Ukraine
